...
/** カードが引っ込むアニメーションの時間。決定の手応えを出しつつ、待たされない長さ。 */
const EXIT_MS = 200

/**
 * スポットを 1 枚ずつ見て、ボタンで「行く」／「見送る」を選び、旅の中身を積み上げていくデッキ。
 * 一覧のチェックボックスだと「今どれを見て何を決めたか」が分かりにくくなるため、
 * 「今この 1 件を決める」ことだけに画面を使う。操作はタップのみ（スワイプは廃止）。
 */
    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="relative mt-5 flex-1" style={{ minHeight: 380 }}>
        {done ? (
          ...
          current && (
            <div
              key={current.id}
              className="anim-rise absolute inset-x-0 top-0 h-full overflow-hidden rounded-3xl border border-white/15 shadow-card"
              style={{
                opacity: exiting ? 0 : 1,
                transform: exiting ? `scale(0.96)` : undefined,
                transition: `opacity ${EXIT_MS}ms var(--e-passage), transform ${EXIT_MS}ms var(--e-passage)`,
              }}
            >
              <Photo src={current.photoUrls[0]} alt={current.name} seed={current.name} className="h-full w-full">
      ...
      {!done && (
        <div className="mt-5 flex items-center justify-center gap-4">
          ...
          <button
            onClick={undo}
            disabled={index === 0 || Boolean(exiting)}
